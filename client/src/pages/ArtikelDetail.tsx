...
import { articles as staticArticles, getRegularArticles } from '../data/articles';
...
export default function ArtikelDetail() {
  ...
  // DB article found
  if (dbArticle) {
    const tags: string[] = Array.isArray(dbArticle.tags) ? dbArticle.tags : [];
    const publishDate = (dbArticle.publishedAt ?? dbArticle.createdAt).toISOString().split('T')[0];
    const imageUrl =
      dbArticle.imageUrl ??
      ...
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <article className="flex-1 max-w-3xl mx-auto w-full px-4 py-12">
          ...
          <nav className="flex items-center gap-2 text-xs text-gray-400 mb-8">
            ...
            <span>/</span>
            <span className="text-gray-600 truncate max-w-[200px]">{dbArticle.title}</span>
          </nav>
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-gray-400 mb-5">
            ...
            <span>·</span>
            <span>{dbArticle.readTime} menit baca</span>
            <span>·</span>
            <span>Oleh {dbArticle.author}</span>
          </div>
          ...
          <div className="prose prose-gray max-w-none text-gray-700 leading-relaxed">
            <Streamdown>{dbArticle.content}</Streamdown>
          </div>
      ...
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <article className="flex-1 max-w-3xl mx-auto w-full px-4 py-12">
          ...
          <nav className="flex items-center gap-2 text-xs text-gray-400 mb-8">
            ...
            <span>/</span>
            <span className="text-gray-600 truncate max-w-[200px]">{staticArticle.title}</span>
          </nav>

          {/* Meta */}
          <div className="flex flex-wrap items-center gap-4 text-xs text-gray-400 mb-5">
            <span>{formatDate(staticArticle.publishDate)}</span>
            <span>·</span>
            <span>{staticArticle.readTime} menit baca</span>
            <span>·</span>
            <span>Oleh {staticArticle.author}</span>
          </div>
          ...
          {/* Tags */}
          {staticArticle.tags && staticArticle.tags.length > 0 && (
            <div className="mt-10 pt-8 border-t border-gray-100">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Tags</p>
              <div className="flex flex-wrap gap-2">
